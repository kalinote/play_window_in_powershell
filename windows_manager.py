...
class WindowsManager:
    ...
    def __init__(self, heigth, width) -> None:
        os.system('cls')
        self.heigth = 0 if heigth < 0 else heigth
        self.width = 0 if width < 0 else width

        for i in range(self.heigth):
            self.vbuffer.append([' '] * self.width)

    def add_window(self, title, background=' '):
        window = Window(
            title=title,
    ...
    def find_window_by_id(self, id):
        return self.total_windows[id]

    def refresh_vbuffer(self):
